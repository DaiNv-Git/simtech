...
/**
 * ✅ CALL_OUT Session - Extract & optimize from GsmService.executeCall
 */
@Slf4j
public class CallOutSession implements TaskSession {
    ...
    public void openPort() throws Exception {
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                log.info("✅ [{}] Port opened for CALL_OUT (attempt {})", comPort, attempt);
                return; // Success
    ...
    public SessionResult execute() throws Exception {
        ...
        try {
            ...
            return SessionResult.builder()
                    ...
                    .taskType("CALL_OUT")
                    .status(callConnected ? "SUCCESS" : "NO_ANSWER")
            ...
            callEndTime = Instant.now();
            log.error("❌ [{}] CALL_OUT failed: {}", comPort, e.getMessage(), e);

            notifyStatus("FAILED");

            return SessionResult.failed(task.getOrderId(), "CALL_OUT", e.getMessage());
        }
    }
    ...
    private String startRecording() throws Exception {
        ...
        String baseFilename = "call_" + comPort.replace("COM", "").replace("/dev/tty", "") + "_" + timestamp;
    ...
    private boolean monitorCallState() throws Exception {
        ...
        for (int i = 0; i < maxAttempts; i++) {
            ...
            // state: 0=active, 2=dialing, 3=alerting
            if (clcc != null && clcc.contains("+CLCC:")) {
                // ✅ Check for active call state (0 in position 3)
                if (clcc.matches(".*,\\d,0,.*")) {
    ...
    private File downloadRecording(String modemPath) throws Exception {
        ...
        String localFilename = "call_" + task.getOrderId() + "_" + System.currentTimeMillis() + ".amr";
